...
* Split mate: extracting candidate rearrangement partners

A split mate is a paired end read in which one mate maps to a
different chromosome than the other mate. This is one form of evidence
for rearrangement candidate.

For rearrangements involving MLL, we are interested particularly in
cases in which one mate maps to the forward strand of chromosome 11,
and another mate maps to a different chromosome.

Note that we don't specify the MLL region of chromosome 11. This is
because another step explicitly maps sequences against the MLL
template. Also, =sampe2db.py= will put "chr11" mates in the first
column, so we don't have to check both mates.
...
These chromosome pairs (chromosome 11 and the chromosomes in the
column =chr_2=) are *rearrangement candidates*.

Note that there is an exception here: certain rearrangements may have
less than a read's length of the alternate chromosome (before the
first Sau3a site occurs). In these cases, there would be one mate
mapped to chromosome 11, but the other mate would be unmapped. This
case is not handled explicitly yet.
