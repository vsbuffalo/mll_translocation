...
## Note that 569 is the position before the first Sau3a site.
if ("cluster-seqs.sam" %in% dir(cluster.aln.dir)) {
...
mc.pos >= smc.lower_pos-400 AND mc.pos <= smc.upper_pos+400;"
  ...
  write.table(top.cands, file.path(results.dir, "top-candidates.txt"), row.names=FALSE, quote=FALSE)
}
